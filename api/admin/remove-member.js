...
module.exports = async (req, res) => {
  try {
    ...
    if (!site_code || !user_id) {
      return json(res, 400, { ok: false, error: "siteCode and userId required" });
    }

    await ensureAdminOrBootstrap(sb, site_code, caller.id);

    // ✅ on retire uniquement l’appartenance au site (on ne supprime pas le compte Auth)
